...
def is_valid_shopee_link(link: str) -> bool:
    return re.match(r"^(https://)?(shp\.id|s\.shopee|www\.shopee\.co\.id|shopee\.co\.id)/.+", link)
...
async def handle_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "ask_shopee_name":
        ...
        if is_valid_shopee_link(message):
            ...
            await update.message.reply_text("✅ Link produk valid.\nJika produk memiliki varian, tulis nama variannya.\nJika tidak ada, ketik: TIDAK ADA")
        else:
            await update.message.reply_text("❌ Link produk tidak valid. Harus dari Shopee.")
